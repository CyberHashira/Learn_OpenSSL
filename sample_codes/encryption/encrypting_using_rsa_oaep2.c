/*      This is a hobby project shared for learning purposes under the MIT License.
	...
	- OAEP will default to sha256, mgf1, and an oaep label.
...
unsigned char *oaepLabel = "Cyber Hashira";
...
void encryptData()
{
	...
	EVP_PKEY_CTX_set_rsa_oaep_md(encCtx, EVP_sha256());
	EVP_PKEY_CTX_set_rsa_mgf1_md(encCtx, EVP_sha256());
	unsigned char *label = strdup(oaepLabel);
	EVP_PKEY_CTX_set0_rsa_oaep_label(encCtx, label, strlen(label));
...
void decryptData()
{
	...
	EVP_PKEY_CTX_set_rsa_padding(decCtx, RSA_PKCS1_OAEP_PADDING);
        EVP_PKEY_CTX_set_rsa_oaep_md(decCtx, EVP_sha256());
        EVP_PKEY_CTX_set_rsa_mgf1_md(decCtx, EVP_sha256());
	unsigned char *label = strdup(oaepLabel);
	EVP_PKEY_CTX_set0_rsa_oaep_label(decCtx, label, strlen(label));
