/*      This is a hobby project shared for learning purposes under the MIT License.
        ...
        OBJECTIVE:
        - This code generates PEM encoded ML-DSA keypair and saves then to a file.
*/
...
void generateKeyPair()
{
	...
	// Get the mldsa context based on the selected variant.
...
void writeToFile()
{
	...
	privateKey = fopen("mldsa-private-key.pem", "w");
	publicKey = fopen("mldsa-public-key.pem", "w");

	PEM_write_PrivateKey(privateKey, mldsa, NULL, NULL, 0, NULL, NULL);
	PEM_write_PUBKEY(publicKey, mldsa);
