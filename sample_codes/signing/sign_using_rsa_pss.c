/*      This is a hobby project shared for learning purposes under the MIT License.
	...
	- It uses RSA-PSS for generating signature.
...
void signData()
{
	...
	EVP_DigestSignInit(mdCtx, &pkeyCtx, EVP_sha256(), NULL, signingKey);

	EVP_PKEY_CTX_set_rsa_padding(pkeyCtx, RSA_PKCS1_PSS_PADDING);
	EVP_PKEY_CTX_set_rsa_mgf1_md(pkeyCtx, EVP_sha256());

	EVP_DigestSign(mdCtx, NULL, &sigLen, dataToSign, fileSize);
	signature = (unsigned char*)malloc(sigLen);
	retVal = EVP_DigestSign(mdCtx, signature, &sigLen, dataToSign, fileSize);
	if(retVal!=1)
	{
		fprintf(stderr, "Failed to sign. Reason : %s.\n", ERR_error_string(ERR_get_error(), NULL));
		exit(1);
	}
	printf(" --> Data signed.\n");
