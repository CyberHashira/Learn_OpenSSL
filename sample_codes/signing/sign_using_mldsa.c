/*      This is a hobby project shared for learning purposes under the MIT License.
	...
	- It uses the MLDSA post-quantum cryptography algorithm for signing the data.
...
void readPrivateKey()
{
	...
	strncpy(mldsaVariant, EVP_PKEY_get0_type_name(signingKey), strlen(EVP_PKEY_get0_type_name(signingKey)));
...
void signData()
{
	EVP_PKEY_CTX *mldsaCtx = EVP_PKEY_CTX_new_from_pkey(NULL, signingKey, NULL);
	...
	EVP_PKEY_sign_message_init(mldsaCtx, sigAlg, mldsaParam);
	EVP_PKEY_sign(mldsaCtx, NULL, &sigLen, dataToSign, fileSize);
	signature = (unsigned char*)malloc(sigLen);
	EVP_PKEY_sign(mldsaCtx, signature, &sigLen, dataToSign, fileSize);
